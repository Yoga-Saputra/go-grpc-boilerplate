...
// Redis interface that is implemented by app
type Redis struct {
	Client *redis.Client
...
func (r *Redis) Close(showLog bool) {
	if r.Client != nil {
...
func (r *Redis) CheckUniqueIsExists(uniqueKey string, exp time.Duration) (bool, error) {
	// Check given key
	if len(uniqueKey) <= 0 {
		return false, errors.New("unique key cannot be empty")
	}
...
func (r *Redis) GetScan(pattern string) (result map[string]interface{}, err error) {
